...
void wait_to_ready_PRIORITY(PCB_Q** return_list, PCB_Q** ready_queue){
    ...
    while(*return_list != NULL){
        add_to_ready_queue_PRIORITY(ready_queue, ((*return_list)->pcb));
...
void add_to_ready_queue_PRIORITY(PCB_Q** ready_queue, PCB* pcb){
    ...
    if(*ready_queue == NULL){
        *ready_queue = new_elem;
        ...
        //moving elem back until priority is higher then current
        while(current_elem != NULL && (current_elem->pcb->priority) >= (new_elem->pcb->priority)){
            previous_elem = current_elem;
            current_elem = current_elem->next_elem;
        }
        //now new elem is higher

        //if new elem's priority is higher than the start
...
void check_wait_PRIORITY(PCB_Q** wait_queue, int cpu_time, PCB_Q** ready_queue_start, int display){
    ...
    wait_to_ready_PRIORITY(return_list, ready_queue_start);
}
...
void check_process_arrive_PRIORITY(PCB_Q** ready_queue, PCB** jobs, int cpu_time, int* next_income, int total_num, int display){
    ...
    if(*next_income >= total_num)
        return;
    if(cpu_time == 0)
        no_check = first_time_check_PRIORITY(arr, total_num);
    while((*next_income < total_num) && ((arr[*next_income].arrival_time) == cpu_time)){
        ...
        add_to_ready_queue_PRIORITY(ready_queue, &arr[*next_income]);
        *next_income += 1;
    }
}
...
int first_time_check_PRIORITY(PCB* arr, int total_num){
    ...
    while(arr[i].arrival_time == 0){
        if(arr[highest].priority < arr[i].priority){
            highest = i;
        }
        i++;
    }
...
void PRIORITY(PCB** jobs, int total_num, int display){
    ...
    *wait_queue = NULL;
    ...
    int highest = first_time_check_PRIORITY(arr, total_num);
    current_pcb = &arr[highest];
